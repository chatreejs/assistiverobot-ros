...
# Set linear and angular values of Kobuki's speed and turning
...
class ControlKobuki():
    def __init__(self):
        # ControlKobuki is name of the node sent to ROS Master
        ...
        # Kobuki will receive message 10 times per second
        rate = rospy.Rate(10)
        ...
        move_cmd.linear.x = 0.1 # Modify value to change speed
        # Turn at 0 radians/s
        move_cmd.angular.z = 0	# Modify value to cause rotation rad/s

        # Loop and Kobuki will move until you type CNTL+c
        while not rospy.is_shutdown():
            # publish the Twist values to Kobuki node /cmd_vel_mux
            self.cmd_vel.publish(move_cmd)
            # wait for 0.1 seconds (10 HZ) and publish again
            rate.sleep()

    def shutdown(self):
        # You can stop Kobuki by publishing an empty Twist message
        rospy.loginfo("Stopping Kobuki")

        self.cmd_vel.publish(Twist())

        # Give Kobuki time to stop
        rospy.sleep(1)
